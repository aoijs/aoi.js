...
class  Database {
  ...
  all(table){
      if(!this.promisify) return this.db.all(table)
      else return new Promise(res=>res(this.db.all(table)))
  }
...
class DbdTsDb extends Database{
    ...
    createSetUp(){
       ...
       if(!this.extraOptions?.dbdtsType){
           ...
           this.tables.forEach(x=>this.db.addTable(new Table(x,this.db,[{name:"id",type:"TEXT"},{name:"invite_tracker",type:"JSON", default:{}}])))
    ...
    get(table,name,id){
        let data = this.db.get(table,{where:{
            column:"id",
            equals:id 
        }}) 
