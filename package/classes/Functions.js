...
class FunctionManager {
    ...
    async cacheFunctions() {
        for (const func of this.functions) {
            try {
                ...
                const file = Object.entries(this.maps).find(y => y[1].includes(ogname))?.[0]
                if (!file) continue;
                else {
                    this.cache.set(ogname, new Function(require(`../functions/Funcs/${file}/${ogname}.js`), func))
                }
            }
            catch (e) {
                console.error(e)
    ...
    createCustomFunction(...ds) {
        for (const d of ds) {
            this.cache.set(d.name.replace("$", ""), new CustomFunction(d, this.client))
            this.functions.push(d.name)
        }
    }
    findFunctions(code = "") {
        let Functions = this.functions
        code = code?.replace(/\\]/g, "#LEFT#").split("\\[").join("#RIGHT#").replace("\\,", "#COMMA#")
        let customFuncs = []
        let funcs = []
        let loadsOfFunc = Functions.filter(thatfunc => code.toLowerCase().includes(thatfunc.toLowerCase()))
        const funcyboys = code.split("$")
