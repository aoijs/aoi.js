...
constructor(client,cache=false){
    super();
        ...
        this.db = new Database ({path:"./database/",tables:["invite"]})
    this.db.connect()
    if(cache){
        this.users = new Collection()
    }

        this.on("FINISHED",()=>{console.log("Fetched All Invites");
...
async fetchAll(){
   ...
   for(let i = 0;i < this.guilds.size;i++){
      const guild = this.guilds.array()[i]
...
this.db.set("invite",`${guild}_${userID}`,data)
    return ;
}
async count(guildID,inviter,type){
 ...
 else { const data = await this.db.get("invite",`${guildID}_${inviter}`)
return data.value.counts[type]||""
       }
}
async lb(custom="{top}) {username}: `total:{total}`,`real:{real}`,`fake:{fake}`,`leave:{leave}`",list=10,page=1){
    let datas;
if(this.cache){
     datas = this.users.array() 
    }
else{
    ...
    for(const d of datas){
     const format = custom.replace(/{top}/g,i)
...
async userJoined(mem){
  const invites = this.invites.get(mem.guild.id).array()
  const newDatas = (await this.fetchInvites(mem.guild)).array()
  for(let i = 0;i < invites.length;i++){
 const index = invites.indexOf(newDatas[i].code)
               
if(invites[index] && invites[index].uses < newDatas[i].uses){
   const inviter =  await this.db.get("invite",`${mem.guild.id}_${invites[index].inviter.id}`) 
if(Number(BigInt(mem.user.createdTimestamp)) >= Number(BigInt(this.fake.limit))){
