...
const { DbdjsApi,DbdTsDb,CustomDB, Promisify}= require('./Database.js')
...
tables: options?.database?.tables||["main"]
     ...
     this.db = new DbdTsDb(options.database?.db,{path:options.database?.path||"./database",tables:options?.database?.tables||["main"]},{type:"dbdts.db", promisify:false},options.database?.extraOptions||{})
 }
...
this.options.ws.properties.$browser = "Discord Android"
     ...
     this.on("ready",async ()=>{require('../Handler/status.js')(this.statuses,this);
