module.exports = async d => {
    ...
    const all = await d.client.db.all(table, x => x.key.split('_').length < 3 && x.key.startsWith(varname.addBrackets()) && !d.util.getGuild(d, x.key.split('_')[1]) && !d.util.getChannel(d, x.key.split('_')[1]))
