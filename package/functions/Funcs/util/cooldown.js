...
module.exports = async d => {
    ...
    let cooldown = await d.client.db.get(d.client.db.tables[0],"cooldown",`${d.command.name}_${d.author.id}_${d.guild.id||"dm"}`);
    cooldown = cooldown?.value ;
    console.log("get from db:"+require("util").inspect(cooldown,{depth:0}));
    if(!cooldown){
        cooldown = Date.now() + Time.parse(time).ms 
        d.client.db.set(d.client.db.tables[0],"cooldown",`${d.command.name}_${d.author.id}_${d.guild.id||"dm"}`,cooldown); 
    }
