...
if(err) return d.error(err) 
    let [channelId,content,embeds="",components="",files="",stickers="",allowedMentions="",reply="",returnId="no"] = inside.splits; 
    content = content === "" ? " ":content
    embeds = embeds === "" ? [] : await EmbedParser(embeds||'') 
    components = components === "" ? [] : await ComponentParser(components||'',d.client);
...
stickers = stickers === "" ?  undefined : stickers.split(",")
   ...
   reply ={ message_id:reply[0] === "" ? undefined : reply[0] } 
    const data = {
        content,embeds, components, allowed_mentions:allowedMentions,message_reference:reply,sticker_ids:stickers 
    } 
  const msg = await d.client.api.channels(channelId).messages.post({
      data,files 
  }).catch(err=>{
  ...
  return {
      code:d.util.setCode({function:d.func,code,inside,result:returnId === "yes"?msg?.id:"" || ""}) 
  }
}