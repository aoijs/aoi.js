...
if(d.client.cmd.awaited.find(y=>y.name.toLowerCase()=== x.toLowerCase())){}
     else{d.aoiError.fnError(d,"custom",{},"Could not find awaitedCommand "+x)} 
     })
...
d.client.interactionManager.on("messageComponentInteraction",interact)
   ...
   if(!cmd) return ;
        await d.interpreter(d.client,interaction,interaction.message.content?.split(" "),cmd,d.client.db,false, undefined,{awaitData: Component.data, interaction})
    })
//--------------------------------------//
