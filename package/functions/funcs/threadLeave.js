...
if(!chan) return d.error(`Invalid ChannelID Provided in ${inside.splits ? `$threadLeave${inside}` : `$threadLeave`}.`);
    try{
...
return {
code:code.replaceLast(inside.splits ? `$threadLeave${inside}` : "$threadLeave", "")
}
  }
    catch(err){
