...
module.exports = async d => {
    ...
    let [index = 1, customId, placeHolder, minValues = 1, maxValues = 1, disabled = "no", ...options] = inside.splits
    ...
    if (!options.length) d.aoiError.fnError(d, "custom", { inside }, "Options Are Not Provided In")
    if (minValues > 25 || minValues < 0) return d.aoiError.fnError(d, "custom", { inside }, "minValues must be between 0 and 25 (both inclusive). Provided Invalid In")
    if (maxValues > 25 || maxValues < 1) return d.aoiError.fnError(d, "custom", { inside }, "maxValues must be between 1 and 25 (both Inclusive). Provided Invalid In")
    if (placeHolder.length > 100) return d.aoiError.fnError(d, "custom", {}, "Placeholder should be at most 100 char long")
