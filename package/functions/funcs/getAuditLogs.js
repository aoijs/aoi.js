module.exports = async d => {
    ...
    const inside = d.unpack()
        ...
        const text = audit.entries.map(logs => format.replace(`{executor.username}`, logs.executor.username).replace(`{executor.mention}`, logs.executor).replace(`{executor.id}`, logs.executor.id).replace(`{executor.tag}`, logs.executor.tag).replace("{target.id}", logs.target.id).replace("{action}", logs.action)).join('\n')
