...
let Options = ["name","id","size","height","width","url"]
     ...
     const channel = d.client.channels.cache.get(channelid)
         ...
         if(!channel) return d.error(`:x: Invalid Channel ID Provided In $getAttachments[${inside.splits.join(" ")}]`)

    if(pos <= 0) return d.error("Position Can't Be Less Than 1")

const msg = await channel.messages.fetch(msgid) 

if(!msg) return d.error(`:x: Invalid Message ID Provided In $getAttachments[${inside.splits.join(" ")}]`)

    

    const option = Options.includes(opt)

    if(!option) return d.error(`:x: Invalid Option Provided In $getAttachments[${inside.splits.join(" ")}]`)
