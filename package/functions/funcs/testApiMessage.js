...
if(!d?.client?.channels?.cache?.get(channel)) return d.error(`$testapiMessageError:Invalid ChannelID Provided.`)  
...
if(msgReply !== ""){
 ...
 let msg =  await axios.post(d.client._api(`channels/${channel}/messages`),
  ...
  data,{
     ...
     }).catch(e=>d.error(`$testapiMessage:${e.message}`))

 return {

     code: code.replaceLast(`$testApiMessage${inside}`,(returnID === "no" ? "" : msg?.data?.id)||"")
