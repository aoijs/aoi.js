...
import { $humanizems } from './$humanizems.js';
import type { ITranspileOptions } from '@aoi.js/typings/interface.js';
import TestCommand from '@aoi.js/testing/testCommand.js';

const client = new TestClient();
client.transpiler.addFunctions({ $humanizems });
...
const codeToFail = '$humanizems';
const codeToPass = '$humanizems[1000]';

void describe('$humanizems', () => {
	...
	void it('should return the humanized time', async () => {
		const { func } = client.transpiler.transpile(codeToPass, transpilerOptions);
		const orignalLog = console.log;
		let logged: unknown;

		console.log = (log: Record<string, string>) => {
			logged = client.parseData(log.content, $humanizems.returns);
		};
