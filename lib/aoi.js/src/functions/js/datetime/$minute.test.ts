...
client.transpiler.addFunctions({ $minute });
...
const codeToPass = '$minute';

void describe('$minute', () => {
	...
	void it('should return the current minute', async () => {
		...
		console.log = (log: Record<string, string>) => {
			logged = client.parseData(log.content, $minute.returns);
		};
