...
client.transpiler.addFunctions({ $month });
...
void describe('$month', () => {
	...
	void it('should return the current month', async () => {
		const { func } = client.transpiler.transpile(codeToPass, transpilerOptions);
		const orignalLog = console.log;
		let logged: unknown;

		console.log = (log: Record<string, string>) => {
			logged = client.parseData(log.content, $month.returns);
		};

		const cur = Intl.DateTimeFormat('en', { month: 'long' }).format(new Date());
