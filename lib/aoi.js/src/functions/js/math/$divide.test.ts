...
client.transpiler.addFunctions({ $divide });
...
const codeToFail = '$divide';
const codeToPass = '$divide[200;200]';
const codeToCalc = '$divide[200;2]';
...
void describe('$divide', () => {
	...
	void it('should return 100 for 200 / 2', async () => {
		...
		console.log = (log: Record<string, string>) => {
			logged = client.parseData(log.content, $divide.returns);
		};

		// @ts-expect-error: func is a function
		await func?.();
		console.log = orignalLog;
		assert.strictEqual(logged, 100);
	});
});
