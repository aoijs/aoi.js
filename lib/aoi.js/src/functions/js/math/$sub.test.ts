...
client.transpiler.addFunctions({ $sub });
...
const codeToFail = '$sub';
const codeToPass = '$sub[200;200]';
const codeToCalc = '$sub[200;2]';
...
void describe('$sub', () => {
	...
	void it('should return 198 for 200 - 2', async () => {
		...
		console.log = (log: Record<string, string>) => {
			logged = client.parseData(log.content, $sub.returns);
		};

		// @ts-expect-error: func is a function
		await func?.();
		console.log = orignalLog;
		assert.strictEqual(logged, 198);
	});
});
