...
import { Bundler, type CommandOptions, type TranspiledFuncData } from '../index.js';
				...
				for (const file of files) {
					...
					if (stats.isDirectory())
						...
						try {
							...
							Commands.push({ path:filePath.split('/').pop()!, loaded:true });
						} catch (e) {
							/* empty */
							Commands.push({ path:filePath.split('/').pop()!, loaded:false, reason:e as string });
						}
					}
				}
			}
