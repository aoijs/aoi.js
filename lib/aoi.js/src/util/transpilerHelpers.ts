 ...
 * @param code - The code to count the brackets in.
 * @returns - Returns an object containing the number of left and right brackets.
 * @example
 * ```js
 ...
 * @param res - The variable name to unescape.
 * @returns The unescaped variable name.
 * @example
 * ```js
 ...
 * @param res - The math result to escape.
 * @returns The escaped math result.
 * @example
 * ```js
 ...
 *
 * @param result - The function result string to wrap.
 * @returns The wrapped function result string.
 *
 * @example
 * ```js
 ...
 * @param code - The code to check.
 * @returns - Returns true if the code has a function, false otherwise.
 * @example
 * ```js
 ...
 * @param code - The code to generate the function data in.
 * @param func - The function to generate the data of.
 * @param functions - The list of valid functions.
 * @param command - The command that the code is in.
 * @returns - Returns the generated function data.
 * @example
 * ```js
 * getFunctionData("$ping", "$ping", ["$ping"]) // { inside: "", total: "$ping", splits: [], funcs: [], parsed: "$ping", type: "getter", code: [Function (anonymous)] }
 * ```
 */
...
export function getFunctionData(
	...
	if (func === '$EXECUTEMAINCODEFUNCTION') {
		funcD = {
			...
			code: (() => {
				/** Empty */
			}) as unknown as FunctionData['code'],
			type: 'scope',
			fields: [
				{
					name: 'code',
	...
	while (true) {
		...
		//     );
		// }
		if (!code.slice(func.length).startsWith('[')) {
			break;
		}
 ...
 * @param code - The code to get the functions in.
 * @param functions - The list of valid functions.
 * @returns - Returns the list of functions.
 * @example
 * ```js
 ...
 * @param arr - The array of function data to reverse.
 * @returns - Returns the reversed array of function data.
 */
 ...
 * @param code - The code to reverse.
 * @param funcs - The function data to reverse.
 * @returns - Returns an object containing the reversed code and function data.
 * @example
 * ```js
 ...
 * @param reverse - Whether to reverse the code and function data or not.
 * @returns - Returns an object containing the generated js function and the scope.
 * @example
 * ```js
...
export function executeData(
	...
	if (reverse) {
		const { code: newCode, funcs } = _reverse(code, data);
		code = newCode;
		data = funcs;
		scope[0].rest = code;
	}
	...
	while (i < data.length) {
		...
		if (d.type === 'scope' || d.type === 'scope_getter') {
			...
			if (d.type === 'scope_getter') {
				d.total = removeFunctionFunction(d.total);
...
export function _parseString(text: string) {
	...
	if (matches) {
		matches = matches.reverse() as RegExpMatchArray;
		let i = 0;
		let u = 0;
 ...
 * @returns - Returns the converted boolean.
 * @example
 * ```js
 ...
 * @param total - The string to remove the customs from.
 * @returns - Returns the string without the customs.
 */
export function removeFunctionFunction(total: string) {
...
export function removeMathFunction(total: string) {
