import { Scope, TranspilerError, Transpiler, parseString, } from "../../..";
import funcs from "../..";
import { getFunctionList, escapeResult, parseData, } from "../../../util/transpilerHelpers.js";
export const $case = {
    name: "$case",
    brackets: true,
    optional: false,
    ...
    code: (data, scope) => {
        ...
        if ($case.brackets) {
            if (!data.total.startsWith($case.name + "[") &&
        ...
        const caseFunctionList = getFunctionList(caseValue, Object.keys(funcs));
        ...
        const newscope = new Scope(`${data.name}_${hash}`, currentScope.name, errorMsg.join(";"), true);
        let executedErrorMsg;
        const errorMsgFunctionList = getFunctionList(errorMsg.join(";"), Object.keys(funcs));
