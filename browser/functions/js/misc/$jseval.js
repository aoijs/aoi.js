import { TranspilerError, parseString } from "../../..";
...
export const $jseval = {
    ...
    code: (data, scope) => {
        ...
        if ($jseval.brackets) {
            if (!data.total.startsWith($jseval.name + "[") &&
        ...
        if (!currentScope.functions.includes("async function __$jseval$__(Code) {")) {
            const setres = `
    async function __$jsEval$__(Code) {
      try {
        ...
        const res = `${escapeResult(`await __$jsEval$__.call(__$DISCORD_DATA$__,${Code})`)}`;
