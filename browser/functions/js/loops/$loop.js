import { TranspilerError, Transpiler, parseStringObject, } from "../../..";
...
export const $loop = {
    ...
    code: (data, scope) => {
        ...
        if (data.inside?.trim() === "" ||
            ...
            throw new TranspilerError(`${data.name} function requires condition and code`);
        }
        ...
        if (isNaN(Number(times)) &&
            !currentScope.name.startsWith("$try_") &&
            !currentScope.name.startsWith("$catch_")) {
            throw new TranspilerError(`${data.name} function requires times field as number`);
        }
        ...
        if (code.join(";").startsWith("{execute:") &&
            code.join(";").endsWith("}")) {
