...
exports.default = (0, createNativeFunction_1.default)({
    name: '$channelNSFW',
    description: 'Gets the current channel nsfw.',
    returns: 'STRING',
    nullable: true,
    fields: [
        {
    ...
    execute: async function (fn) {
        if (fn.hasFields()) {
            return this.manage(await fn.resolveArray(this), async ([ch]) => {
                return structures_1.Return.string(ch.isDMBased() ? '' : ch.nsfw);
            });
        }
        const ch = this.context.getMainChannel();
        return structures_1.Return.string(ch && !ch.isDMBased() ? ch.nsfw : '');
    }
});
