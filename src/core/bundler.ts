...
export function Bundler(code: string) {
    ...
    command.code = Transpiler.Transpiler(<string>command.code, {
        scopeData:{
            embedJs: embedJs,
        },
        sendMessage: true,
...
export function parseExportCommand(s:string) {
    const commandTypeMatchArray = s.match(/\[exportCommand:\s*(\w+)\]/);
    ...
    obj.code = (<string>obj.code).replaceAll(BundlerCustoms.LB,"{").replaceAll(BundlerCustoms.RB, "}");
    return obj;
}
