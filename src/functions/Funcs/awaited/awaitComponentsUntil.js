...
module.exports = async (d) => {
    ...
    let chan = await d.util.getChannel(channelId)
    ...
    awaitCommands.forEach(x => {
        if (d.client.cmd.awaited.find(y => y.name.toLowerCase() === x.toLowerCase())) { }
            ...
            return d.aoiError.fnError(d, "custom", {}, `Could Not Found AwaitedCommand:${x} `)
        }
    })
    if (data !== "") {
        try {
            ...
            return d.aoiError.fnError(d, "custom", { inside }, `Invalid Data Format Provided In ${inside}`)
        }
    }
    ...
    msg.awaitMessageComponent({ filter, time }).then(async int => {
        ...
        if (errorMsg !== "") {
            ...
            d.aoiError.makeMessageError(d.client, d.channel, errorMsg, extraOptions)
        }
        else {
