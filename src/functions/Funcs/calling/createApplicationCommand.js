...
module.exports = async d => {
    ...
    let [guildId, name, description, defaultPermission = "yes", type = "slash", ...opts] = inside.splits;
    name = name.addBrackets()
    let options;
    let data;
    const guild = guildId === "global" ? undefined : guildId === "custom" ? "custom" : await d.util.getGuild(d, guildId);
    if (!guild && !["global", "custom"].includes(guildId)) return d.aoiError.fnError(d, "guild", {inside});
