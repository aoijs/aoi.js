...
module.exports = async (d) => {
	...
	if (eval(CheckCondition.solve(mustEscape(condition)))) {
		if (trueawait.addBrackets().includes("{execute:")) {
			...
			if (!cmd)
				return d.aoiError.fnError(
					...
					"custom",
					{},
					"Invalid Awaited Command: '" +
						trueawait.addBrackets().split("{execute:")[1].split("}")[0] +
						"' Provided",
				);
			await d.interpreter(
				d.client,
		...
		if (falseawait?.addBrackets().includes("{execute:")) {
			...
			if (!cmd)
				d.aoiError.fnError(
					...
					{},
					"Invalid Awaited Command: " +
						falseawait.addBrackets().split("{execute:")[1].split("}")[0] +
						" Provided",
				);
			await d.interpreter(
				d.client,
		...
		} else data.result = falseawait.addBrackets();
	}
