module.exports = d => {
    ...
    if (times === 'all') {
        data.result = text.addBrackets().replaceAll(replacer.addBrackets(), replacedTo.addBrackets()).deleteBrackets();
    }
    else {
        if (Number(times) === 0) data.result = text.deleteBrackets();
        else if (Number(times) === 1) {
            data.result = text.addBrackets().replace(replacer.addBrackets(), replacedTo.addBrackets()).deleteBrackets();
        }
        else {
            ...
            if (Number(times) > 0) {
                slice = replacer.addBrackets() + arr.slice(time).join(replacer.addBrackets())
            }
            else {
