...
module.exports = async d => {
    ...
    let cooldown = await d.client.db.get(d.client.db.tables[0],"cooldown",`${d.command.name}_${d.channel.id}`);
    
    if(!cooldown){
        cooldown = Date.now() + Time.parse(time).ms 
        d.client.db.set(d.client.db.tables[0],"cooldown",`${d.command.name}_${d.channel.id}`,cooldown); 
    }
