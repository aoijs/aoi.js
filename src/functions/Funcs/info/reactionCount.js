module.exports = async d => {
    ...
    const emoji = message.reactions.cache.find(x => x.emoji.id === emojiResolver || x.emoji.toString().toLowerCase() === emojiResolver.addBrackets().toLowerCase());
    if (!emoji) return d.aoiError.fnError(d, 'emoji', { inside: data.inside });

    data.result = emoji.size;
