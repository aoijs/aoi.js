...
module.exports = async (d) => {
    ...
    roles.forEach((x) => {
        if (!guild.roles.cache.find((y) => y.name.toLowerCase() === x.toLowerCase().addBrackets() || y.id === x)) wrongRoles.push(x);
        else ros.push(guild.roles.cache.find((y) => y.name.toLowerCase() === x.toLowerCase().addBrackets() || y.id === x));
    });
