...
module.exports = async (d) => {
    ...
    if (d.util.isUnicodeEmoji(name)) {
        return d.aoiError.fnError(d, "custom", { inside: data.inside }, "Invalid Emoji");
    }
    ...
    if (id === "global") {
        ...
        data.result = d.client.guilds.cache
            .get(id)
            ?.emojis.cache.find((x) => x.name.toLowerCase() === name.toLowerCase() || x.toString() === name || x.id === name)
            ?.toString();
    }
