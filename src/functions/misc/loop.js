...
module.exports = async (d) => {
    ...
    awaits.forEach((cmd) => {
        if (!d.client.cmd.awaited.find((x) => x.name.toLowerCase() === cmd.addBrackets().toLowerCase()))
            return d.aoiError.fnError(d, "custom", { inside: data.inside }, "Awaited Command : " + cmd + " Not Found");
    });
