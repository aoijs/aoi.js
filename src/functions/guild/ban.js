...
module.exports = async (d) => {
    ...
    if (isNaN(deleteMessageDays) || deleteMessageDays > 7 || deleteMessageDays < 0) return d.aoiError.fnError(d, "custom", { inside: data.inside }, "Invalid Day Provided In");

    deleteMessageSeconds = deleteMessageDays * 24 * 60 * 60;

    guild.members.ban(userID, { deleteMessageSeconds, reason: reason?.addBrackets() }).catch((error) => {
