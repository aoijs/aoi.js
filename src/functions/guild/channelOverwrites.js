...
module.exports = async (d) => {
    ...
    data.result = overwrites
        .map((x) =>
            response
                .replaceAll("{mention}", x.type === "role" ? `<@&${x.id}>` : `<@&${x.id}>`)
                .replaceAll("{type}", x.type)
                .replaceAll("{allow}", x.allow.toArray().join(" , "))
                .replaceAll("{deny}", x.deny.toArray().join(" , "))
        )
        .join(sep);
