...
module.exports = async (d) => {
    ...
    if (createAt instanceof Guild) {
        channel = await createAt.channels.create({ name: name.addBrackets() }).catch(async (err) => {
            if (error && error?.trim() !== "") {
                const ticketError = await d.util.errorParser(error.addBrackets(), d);
                await d.aoiError.makeMessageError(d.client, d.channel, ticketError.data ?? ticketError, ticketError.options);
            } else {
        ...
        channel = await createAt.children.create({ name: name.addBrackets() }).catch(async (err) => {
            if (error && error?.trim() !== "") {
    ...
    if (channel) {
        await d.client.db.set("__aoijs_vars__", "ticketChannel", channel.id, channel.id);
        ...
        if (message && message.trim() !== "") {
            ...
            try {
                await d.aoiError.makeMessageError(d.client, channel, ticketMsg.data ?? ticketMsg, ticketMsg.options);
            } catch (err) {
