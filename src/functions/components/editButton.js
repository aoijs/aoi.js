...
module.exports = async (d) => {
    ...
    const index = components.findIndex((row) => row.components.some((button) => button.customId === customId));
    ...
    if (index !== -1) {
        ...
        const newButtons = row.components.map((button) => {
            if (button.customId === customId) {
                ...
                style != 5 ? newButton.setCustomId(customId) : newButton.setURL(custom || button.url);
                return newButton;
            } else {
                return button;
            }
