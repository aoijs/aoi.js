...
module.exports = async (d) => {
    ...
    let [userfilter, time, reactions, commands, errorMsg = "", awaitData = ""] = data.inside.splits;

    reactions = reactions.split(",");

    const filter = (r, u) => {
        return (
            (userfilter === "everyone" ? u.id !== d.client.user.id : u.id === userfilter) &&
            (reactions.includes(r.emoji.toString()) || reactions.includes(r.emoji.name) || reactions.includes(r.emoji.id))
        );
    };
    ...
    d.message
        ...
        .then(async (collected) => {
            ...
            const index = reactions.findIndex((r) => (r === collected.emoji?.toString()) | (r === collected.emoji?.id) || r.toLowerCase() === collected.emoji?.name.toLowerCase());
            const cmd = d.client.cmd.awaited.find((x) => x.name.toLowerCase() === commands[index]?.toLowerCase());
            if (!cmd) return d.aoiError.fnError(d, "custom", {}, "Invalid Await Command: " + commands[index]);
            await d.interpreter(d.client, d.message, d.args, cmd, d.client.db, false, undefined, { ...d.data, awaitData: awaitData });
        })
        .catch(async (err) => {
            if (errorMsg !== "") {
