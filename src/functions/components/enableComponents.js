    ...
    const ActionRow = components.map((row) => {
        const newButtons = row.components.map((component) => {
            if (component.type === 2 && (type.toLowerCase() === "button" || type.toLowerCase() === "all")) {
                ...
                if (component.type === 3) {
                    return StringSelectMenuBuilder.from(component).setDisabled(false);
                } else if (component.type === 5) {
                    return UserSelectMenuBuilder.from(component).setDisabled(false);
                } else if (component.type === 6) {
                    return RoleSelectMenuBuilder.from(component).setDisabled(false);
                } else if (component.type === 7) {
                    return MentionableSelectMenuBuilder.from(component).setDisabled(false);
                } else if (component.type === 8) {
                    return ChannelSelectMenuBuilder.from(component).setDisabled(false);
                } else {
                    return component;
                }
            } else {
