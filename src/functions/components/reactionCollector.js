...
module.exports = async (d) => {
    ...
    userFilters = userFilters === "everyone" ? "everyone" : userFilters.split(",").map((x) => x.trim());
    ...
    const filter = (reaction, user) =>
        (userFilters === "everyone" ? user.id !== d.client.user.id : userFilters.includes(user.id)) && reactions.includes(reaction.emoji.toString() || reaction.emoji.id);
    ...
    collector.on("collect", async (reaction, user) => {
        ...
        const index = reactions.indexOf(reaction.emoji.id) === -1 ? reactions.indexOf(reaction.emoji.toString()) : reactions.indexOf(reaction.emoji.id);
    ...
    if (endAwait) {
        collector.on("finish", async (reaction) => {
            const endCmd = d.client.cmd.awaited.find((x) => x.name.toLowerCase() === endAwait.trim().addBrackets().toLowerCase());
            if (!endCmd) return;
            await d.interpreter(d.client, d.message, [], d.client.db, false, undefined, { awaitData });
        });
    }
